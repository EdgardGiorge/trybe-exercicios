...
//É comum encontrar em JavaScript linhas de código que possuem comportamentos assíncronos. Você já conhece dois casos em que comportamentos assíncronos acontecem: callbacks e async/await . Para testar esses casos, o Jest fornece algumas soluções com o objetivo de que os testes aguardem o momento em que a função a ser testada for concluída e a informação necessária for retornada, para depois executar os testes. Isso evita que falsos positivos (teste passar quando deveria falhar) aconteçam e garante segurança para a aplicação. Hoje, você vai focar os estudos utilizando o async/await :
...
//A seguir você conhecerá de forma detalhada o conceito apresentado acima. Não se preocupe, existem exemplos e exercícios para que você possa praticar e consolidar o aprendizado! 😉
...
//  1- A URL da API estar incorreta.
//  2- Uma falha na conexão com a internet.
//  3- Uma instabilidade no servidor que gerencia a API.
...
//Apareceu um erro no seu terminal, certo? 👻 Não se preocupe se o texto do erro for grande, observe a primeira linha:
...
//Até aqui foi testado o resultado da requisição, mas como testar o valor da promise, ou seja, esperar que a promise seja resolvida ou rejeitada? Existem dois matchers, o resolves e o rejects que fazem o teste direto na promise.
...
//Exemplo do video usando resolves e rejects
